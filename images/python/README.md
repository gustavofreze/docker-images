...
|:-------------------------|:-------------------|:--------------|:--------------------------------------------------------------|
| `3.14-runtime-1.0.2`     | `3.14-runtime`     | `runtime`     | Production interpreter. No compiler, no installer, no Poetry. |
| `3.14-development-1.0.2` | `3.14-development` | `development` | Development runtime: runtime plus Poetry, debugpy, bash, git. |
| `3.14-builder-1.0.2`     | `3.14-builder`     | `builder`     | Poetry and the C toolchain for the builder stage of an image. |
| `3.14-cli-1.0.2`         | `3.14-cli`         | `cli`         | Makefile tooling: Poetry, git, bash, curl.                    |
...
FROM gustavofreze/python:3.14-development-1.0.2
COPY ./ /app
```

```dockerfile
# syntax=docker/dockerfile:1
FROM gustavofreze/python:3.14-builder-1.0.2 AS builder
COPY pyproject.toml poetry.lock ./
RUN poetry install --no-root --only main
COPY ./ ./

FROM gustavofreze/python:3.14-runtime-1.0.2
...
```shell
docker run --rm -v "$(pwd)":/app gustavofreze/python:3.14-cli-1.0.2 poetry install
docker run --rm -v "$(pwd)":/app gustavofreze/python:3.14-cli-1.0.2 python3 your-script.py
```
...
```shell
trivy image --vex oci gustavofreze/python:3.14-runtime-1.0.2
```
