...
1. Every Dockerfile, base image or consumer, begins with `# syntax=docker/dockerfile:1` on its literal first line. See §
   Syntax.
...
`<upstream-minor>-<role>` that the weekly security rebuild repoints at a freshly pulled upstream base. A project always
pins the full versioned tag.
...
- **The Python `runtime` uninstalls pip on purpose.** It is the largest dependency tree in the image and nothing in it
  ...
  the way back. `scan-python` audits this target with no ignore file, so restoring pip here fails the gate.
...
any input change bumps `VERSION`. A vulnerability finding that cannot be fixed locally is accepted only through a
suppression scoped to the single family it affects, carrying a written justification and a revisit condition, never a
repository-wide one.
